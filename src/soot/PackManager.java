...
public class PackManager {
    ...
    private void init()
    {
        ...
        addPack(p = new ScenePack("wjtp"));
        {
	    	...
	    	p.add(new Transform("wjtp.tnlp", LockProducer.v()));
	...
	public static boolean ignoreTransform= true;
...
//        } else 
	...
	public  HashMap<SootMethod, Body> m2b = new  HashMap<SootMethod, Body>();
    
    public void checkpointBodies() {
        Iterator clIt = Scene.v().getApplicationClasses().iterator();// reachableClasses();
        while( clIt.hasNext() ) {
            SootClass cl = (SootClass) clIt.next();
            Iterator methodIt = cl.methodIterator();
            while (methodIt.hasNext()) {
