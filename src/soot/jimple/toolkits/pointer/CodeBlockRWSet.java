...
*/					
    ...
    public boolean union( RWSet other )// return represent whether the native is contained. true=> contained
    {
    ...
    public CodeBlockRWSet minus( MethodRWSet other )
    {// May run slowly... O(n^2)
		CodeBlockRWSet ret = new CodeBlockRWSet();
			...
			&& !fields.isEmpty() && !other.fields.isEmpty() )
		{
		    for (Object element : fields.keySet()) {
				...
				if( !other.fields.containsKey( field ) ) 
				{
					PointsToSet pts1 = getBaseForField( field );;
					ret.addFieldRef(pts1, field);
				}
					...
					//!!!!!!!!!!!!problematic
					// you can get all allocnodes, and really do the minus then.
					//I wont do it now
					if(pts2 instanceof FullObjectSet)
					{
						//PointsToSetInternal empty = new  HashPointsToSet(RefType.v("java.lang.Object"),(PAG) Scene.v().getPointsToAnalysis());
						//ret.addFieldRef(empty, field);
						// do not put it at all, or else, will return as if the field still exists!
					}
					else if(pts1 instanceof FullObjectSet)
						ret.addFieldRef(pts1, field);// mei ban fa, hai shi full representation
