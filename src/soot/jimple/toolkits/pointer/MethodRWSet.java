...
public class MethodRWSet extends RWSet {
    ...
    public boolean hasNonEmptyInterestingIntersection( RWSet oth ) {
    	if( isFull ) return oth != null;
    	if( !(oth instanceof MethodRWSet) ) {
    	    return oth.hasNonEmptyIntersection( this );
    	}
    	...
    	if( fields != null && other.fields != null
    	    ...
    	    for (Object element : other.fields.keySet()) {
    	        final Object field = element;
    		if( fields.containsKey( field ) ) {
    		    if( hasNonEmptyInterestingIntersection(
                                    getBaseForField( field ),
    				other.getBaseForField( field ) ) ) {
    			return true;
    		    }
    ...
    public static boolean hasNonEmptyInterestingIntersection(PointsToSet currentPara,PointsToSet otherPara  )
    {
    	final PointsToSetInternal current = (PointsToSetInternal) currentPara;
    	final PointsToSetInternal other  = (PointsToSetInternal) otherPara;
    	P2SetVisitor pSetVisitor =new P2SetVisitor() {
            public void visit( Node n ) {
            	...
            	{
            		RefType refType = (RefType)n.getType();    
            		SootClass exception = Scene.v().loadClassAndSupport("java.lang.Throwable");            		

            	    FastHierarchy fHierarchy =Scene.v().getFastHierarchy();
            	    if(fHierarchy.canStoreType(refType, exception.getType()))
            	    {
            	    	return;// filter out
            	    }
            	}
    	...
    	other.forall( pSetVisitor );
    	return pSetVisitor.getReturnValue();
