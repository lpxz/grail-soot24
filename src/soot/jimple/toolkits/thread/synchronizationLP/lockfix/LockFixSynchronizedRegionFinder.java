 ...
 */
public class LockFixSynchronizedRegionFinder extends ForwardFlowAnalysis<Unit, FlowSet>
{
...
//	SideEffectAnalysis sea;
    ...
    LockFixSynchronizedRegionFinder(UnitGraph graph, Body b, boolean optionPrintDebug, boolean optionOpenNesting, ThreadLocalObjectsAnalysis tlo)
	{
		super(graph);
    ...
    protected FlowSet newInitialFlow()
    {
		...
		if(method.isSynchronized() && methodTn != null)
		{
			ret.add(new LockFixSynchronizedRegionFlowPair(methodTn, true));
		}
        return ret;
    }
    ...
    /**
     * OUT is the same as (IN minus kil lSet) plus the genSet.
     **/
    protected void flowThrough(FlowSet in, Unit unit, FlowSet out)
    {
		...
		int nestLevel = 0;
		CriticalSection mostInTX= null;
        Iterator outIt0 = out.iterator();
        while(outIt0.hasNext())
        {
            LockFixSynchronizedRegionFlowPair srfp = (LockFixSynchronizedRegionFlowPair) outIt0.next();
            if(srfp.tn.nestLevel > nestLevel && srfp.inside == true)
            	{
            	   nestLevel = srfp.tn.nestLevel;
            	   mostInTX = srfp.tn;
            	}
        }// very funny here: the most "Inner" one will win, and tell the newly created Tx his level is nestLevel+1;
        ...
        while(outIt.hasNext())
        {
            LockFixSynchronizedRegionFlowPair srfp = (LockFixSynchronizedRegionFlowPair) outIt.next();
        	...
        	if(srfp.inside == true && (tn.nestLevel == nestLevel || optionOpenNesting == false))
        	{
            	...
            	else if(unit instanceof ExitMonitorStmt && tn.nestLevel == nestLevel) // only applies to outermost txn, actually "you" mean the innermost here
            	{ // e.g., tx.level(=1) can not do this for himself, while tx.level(=2) can do as it is the most inner one
				...
				else
            	{
        			...
        			//===========
        		     ...
        		     tn.addBase4Stmt(stmt, uses);// base = use
        			//===========
        ...
        if(addSelf)
        {
			...
			out.add(new LockFixSynchronizedRegionFlowPair(newTn, true));
    ...
    protected void copy(FlowSet sourceSet, FlowSet destSet)
    {
		...
		while(it.hasNext())
		{
			LockFixSynchronizedRegionFlowPair tfp = (LockFixSynchronizedRegionFlowPair) it.next();
