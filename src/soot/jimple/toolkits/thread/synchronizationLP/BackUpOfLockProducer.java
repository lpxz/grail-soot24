...
public class BackUpOfLockProducer extends SceneTransformer
{
    public BackUpOfLockProducer(Singletons.Global g){}
    public static ThreadLocalObjectsAnalysis g_tlo =null ;
//    public static BackUpOfLockProducer v() 
//	{ 
...
//====================================================================  
//=========================== outermost css
    	List<CriticalSection> outerMostCSList = new ArrayList<CriticalSection>();
    	for(int i= 0; i<criticalSections.size();i++)
    	{
    		CriticalSection item = criticalSections.get(i);
    		if(item.parent ==null)
    		{
    			// yes, this is the outermost TN
...
//=========================== inter-component conflicting detection
       	for(int i=0;i< comps_variant.size(); i++)
       	{
       		...
       		detectConflictsForEachCS(compAndSelf);// put it into the static field!
       	}
...
//============================print out the jimple and groups together    	
    	...
    	// commented temporarily. maybe permantently..
//    	//LP: inspect the conflicting data: a data race detector actually.
//    	//mayHappenInParallel(tn1,tn2)  @link CriticalSectionInterferenceGraph
//        mhpCriticalSections= new ArrayList<CriticalSection>();
//    	for(int i=0;i<criticalSections.size();i++)
//    	{
//    		CriticalSection cs = criticalSections.get(i);
//    		mhpCriticalSections.add(cs);
//    		if(mhp.mayHappenInParallel(cs.method, cs.method))// like the thread, add additional copy
//    		{
//    			mhpCriticalSections.add(cs);    			
//    		}
//    	}
//    	
//    	// here, we collect the info of conflicting data:
//    	// data, isread, myTx, conflictingTxAndOverlaps(conflictingTX and the reasons)
//    	// this is a complete  summary. 
//    	Iterator tnItNew = mhpCriticalSections.iterator();
//        while(tnItNew.hasNext())// for same cs, there may two routines
//    	{// foreach transaction
//    		// tn.read,stmtRead: CodeBlockRWSet: fields: field-> pointtoset
//    		
//    		CriticalSection tn = (CriticalSection) tnItNew.next();
//    		CodeBlockRWSet readRWSet= tn.read;
//    		CodeBlockRWSet writeRWSet= tn.write;
//    		
//    		HashSet<ConflictingData> tmpConflictingDatas=null;
//    		tmpConflictingDatas= readsConflicted(readRWSet,tn);
//    		tn.conflictingDatas.addAll(tmpConflictingDatas);
//    		tmpConflictingDatas= writesConflicted(writeRWSet,tn);
//    		tn.conflictingDatas.addAll(tmpConflictingDatas);   		
//    		
//    		
//    	}        
//        for(int i= 0;i<criticalSections.size();i++)// notice we do not use mhpciriticalsections here. no need to repeat.
//        {
//        	 CriticalSection tn = criticalSections.get(i);
//        	 tn.conflictingDatas= reduceRedundent(tn.conflictingDatas);
//        }
       
//================================================================
        
        
         // one-line transaction for read/write sets
    	// *** Find Stray Reads/Writes *** (DISABLED) // yes, they do notice this information
...
//    	}
	...
	private HashSet<ConflictingData> reduceRedundent(
			HashSet<ConflictingData> conflictingFields) {
		HashSet<ConflictingData> ret = new HashSet<ConflictingData>();
		ConflictingData[] toList = new ConflictingData[conflictingFields.size()];// ordered
		conflictingFields.toArray(toList);
		for(int i=0;i<toList.length;i++)// I did not collapse the structure, haha
		{
			ConflictingData bigbrother = toList[i];
			if(bigbrother==null) continue; // not a bigbrother at all
			for(int j=i+1; j<toList.length;j++)
			{
				ConflictingData smallbrother = toList[j];
				if(smallbrother==null) continue; // this smallbrother has been adopted
				if(bigbrother.sameRepAs(smallbrother))
				{
					bigbrother.conflictingTXandOverlaps.addAll(smallbrother.conflictingTXandOverlaps);
					toList[j]= null;
					ret.add(bigbrother);// bigbrother is put into the history.
					// add to ret, 
				}
			}
		}
	...
	private HashSet<ConflictingData> writesConflicted(
			CodeBlockRWSet writeRWSet, CriticalSection tn) {
		HashSet<ConflictingData> retSet = new HashSet<ConflictingData>();
		if(writeRWSet.fields==null) return retSet;
		Iterator it = writeRWSet.fields.keySet().iterator();
		while (it.hasNext()) {
			Object inspectedData = (Object) it.next();
			ConflictingData conflictingData = new ConflictingData();
			conflictingData.data = inspectedData;
			conflictingData.mytn=tn;
			conflictingData.iamRead= false;
			
			PointsToSet inspected_point_to= writeRWSet.fields.get(inspectedData);
            List TxsExceptMe= getTXsExceptMe(tn);
            for(int i=0;i<TxsExceptMe.size();i++)
            {
            	CriticalSection otherTXTmp=(CriticalSection) TxsExceptMe.get(i);
            	CodeBlockRWSet otherTXreads = otherTXTmp.read;
            	if(otherTXreads.fields==null) continue;
                PointsToSet	other_point_to_read= otherTXreads.fields.get(inspectedData);
                if(inspected_point_to.hasNonEmptyIntersection(other_point_to_read))
                {
                	TxAndOverlap txAndOverlap = new TxAndOverlap();
                	txAndOverlap.tx =otherTXTmp;
                	txAndOverlap.overlappedPointsToSet= ((PointsToSetInternal)inspected_point_to).intersectWith(other_point_to_read);
                	
                	conflictingData.conflictingTXandOverlaps.add(txAndOverlap);
                	continue; // one is enough, read or ok, register once for the same tx.
                }
                CodeBlockRWSet otherTXwrites = otherTXTmp.write;
                if(otherTXwrites.fields==null) continue;
                PointsToSet other_point_to_write= otherTXwrites.fields.get(inspectedData);
                if(inspected_point_to.hasNonEmptyIntersection(other_point_to_write))
                {
                	TxAndOverlap txAndOverlap = new TxAndOverlap();
                	txAndOverlap.tx= otherTXTmp;
                	txAndOverlap.overlappedPointsToSet= ((PointsToSetInternal)inspected_point_to).intersectWith(other_point_to_write);
                	
                	conflictingData.conflictingTXandOverlaps.add(txAndOverlap);
                    continue;
                }
            }
            if(conflictingData.conflictingTXandOverlaps.size()!=0)
            {
                retSet.add(conflictingData);
            }
	...
	private List getTXsExceptMe(CriticalSection tn) {
	    List allTXs= mhpCriticalSections;
	    List ret = new ArrayList<CriticalSection>();
        ret.addAll(allTXs);
        ret.remove(tn);// remove the first occurence, this is wanted
        // there may be cs1, cs1 in the list for the self-conflicting.
        
		return ret;
	}
	private HashSet<ConflictingData> readsConflicted(CodeBlockRWSet readRWSet,
			CriticalSection tn) {
		HashSet<ConflictingData> retSet = new HashSet<ConflictingData>();
//		Exception in thread "Main Thread" java.lang.NullPointerException
//		at soot.jimple.toolkits.thread.synchronizationLP.LockProducer.readsConflicted(LockProducer.java:552)
//		at soot.jimple.toolkits.thread.synchronizationLP.LockProducer.internalTransform(LockProducer.java:290)
...
//		at Drivers.RelativeGroupAnalysis.main(RelativeGroupAnalysis.java:98)

		// note readRWSet is tn's field
		if(readRWSet.fields==null) return retSet;// I have no readset, so no confliction on this point
		Iterator it = readRWSet.fields.keySet().iterator();// null exception
		while (it.hasNext()) {
			Object inspectedData = (Object) it.next();
			PointsToSet inspectedPoint2S= readRWSet.fields.get(inspectedData);
			ConflictingData conflictingData = new ConflictingData();
			conflictingData.data = inspectedData;
			conflictingData.mytn=tn;
			conflictingData.iamRead= true;
...
//			ThreadLocalObjectsAnalysis tlo= new ThreadLocalObjectsAnalysis(new SynchObliviousMhpAnalysis());
			...
			for(int i=0;i<TxsExceptMe.size();i++)
			{
				CriticalSection otherTX = (CriticalSection)TxsExceptMe.get(i);
			    CodeBlockRWSet otherwrites = otherTX.write;
			    if(otherwrites.fields==null) continue;
			    PointsToSet other_point_to =otherwrites.fields.get(inspectedData);
			    if(inspectedPoint2S.hasNonEmptyIntersection(other_point_to))
			    {
			    	// add the complete info: intersected point-to , the tn
			    	TxAndOverlap txAndOverlap = new TxAndOverlap();
			    	txAndOverlap.tx=otherTX;
			    	txAndOverlap.overlappedPointsToSet= ((PointsToSetInternal)inspectedPoint2S).intersectWith(other_point_to);
