...
//		this.unitToUses = (HashMap<Unit, List>) tn.unitToUses.clone();
	...
	{
		String classname  =this.method.getDeclaringClass().getName();
	...
	public void addCSDDSet4Stmt(Unit csUnit, CriticalSectionDataDependency csdd) {
	      if(csdd==null) return;
	      if(!unitToCSDDs.containsKey(csUnit))
	      {
	    	  Set<CriticalSectionDataDependency> creatIt = new HashSet<CriticalSectionDataDependency>();
	    	  unitToCSDDs.put(csUnit, creatIt);
	      }
	      Set<CriticalSectionDataDependency> container = unitToCSDDs.get(csUnit);
	      container.add(csdd);	
	}
	...
	public void addClassesForUnit(Unit csUnit, Set<SootClass> allClassesInUnit) {
		if(!unitToClasses.containsKey(csUnit))
		{
			...
			if(unitToClasses.get(csUnit)==allClassesInUnit) return;
				...
				if(sameContent(unitToClasses.get(csUnit), allClassesInUnit))
				{
					return;
				}
				else {
					// should not pass this path, because we only add once for a unit in a tn
					System.out.println("what is up? do you need to add twice for the smae  unit in a tn? and each time is a bit different");
					Assert.assertTrue(1==0);
				}
			}			
		}		
	...
	private boolean sameContent(Set<SootClass> set,
			Set<SootClass> allClassesInUnit) {
	   boolean bool1 =  belongTo(set,allClassesInUnit);
		boolean bool2 = belongTo(allClassesInUnit,set);
	...
	private boolean belongTo(Set<SootClass> set, Set<SootClass> allClassesInUnit) {
		boolean ret= true;
		for(SootClass sc:set)
		{
			if(!allClassesInUnit.contains(sc))
			{
				ret =false;
			}
		}
