...
import pldi.locking.studyConcurrent.SootMain;
...
public class HashMutablePDG extends HashMutableEdgeLabelledDirectedGraph implements ProgramDependenceGraph {
	...
	public BlockGraph m_blockCFG = null;
	public Hashtable<Object, PDGNode> m_obj2pdgNode = new Hashtable<Object, PDGNode>();
		...
		while(!regions2process.isEmpty())
		{
			...
			for(Iterator<Block> itr = blocks.iterator(); itr.hasNext();)
			{
				...
				for(Iterator<Block> bItr = bs.iterator(); bItr.hasNext();)
				{
					...
					dependents.clear();// reusable data structure.
					Block b = bItr.next();
					...
					String[] items =aParentDode.toString().split("\n");
					if(items.length==3 && items[2].contains("nop;"))
					{
						//do not enter the while loop, outofmemory, endless
					}
					else {
						// this code is dead, why does the developer leave it here?
							...
							label :	while(succItr.hasNext())
									{
										...
										if(succRPDGNode.getType() == PDGNode.Type.REGION)
										{
										...
										else if(succRPDGNode.getType() == PDGNode.Type.CFGNODE){
											Block h= (Block) succRPDGNode.getNode();
											DominatorNode hdode = dom.getDode(h);
											DominatorNode adode = dom.getDode(a);
											...
											throw new RuntimeException("type unexpected");
										}
									}
									
									if(loopBodyPDGNode==null) throw new RuntimeException("unexpected");
									((LoopedPDGNode)newpdgnode).setBody(loopBodyPDGNode);
	...
	private static List<PDGRegion> computePDGRegions(PDGNode root)
	{
		...
		node2Region.clear();
		if(root.getType().equals(PDGNode.Type.REGION))
		    pdgpostorder(root, regions);
		else if (root.getType().equals(PDGNode.Type.CFGNODE))  // lpxz, it is possibly block!			
		{			
			Body bb = ((Block)root.getNode()).getBody();
			SootMethod sm = bb.getMethod();
			SootClass sc = sm.getDeclaringClass();
			PDGRegion region = new PDGRegion(root.hashCode(), sm,sc, new EnhancedUnitGraph(bb), root);
			regions.add(region);
		}
	...
	static Set visited = new HashSet();//lpxz
	private static PDGRegion pdgpostorder(PDGNode node, List<PDGRegion> list)
	{ // be used to set children and parents
		visited.clear();
		visited.add(node);
			...
			for(Iterator<PDGNode> itr = dependents.iterator(); itr.hasNext(); )
			{
				...
				if(curNode instanceof LoopedPDGNode)
				{
					...
					if(!visited.contains(body))
					{
						visited.add(body);
						PDGRegion kid= null;
						if(body.getType().equals(PDGNode.Type.REGION))
						{
							kid = pdgpostorder(body, list);
							kid.setParent(region);
							region.addChildRegion(kid);
							
							//This sets the node from the old Region into a PDGRegion
							body.setNode(kid);
						}else if (body.getType().equals(PDGNode.Type.CFGNODE)) {
							Body bb = ((Block)body.getNode()).getBody();
							SootMethod sm = bb.getMethod();
							SootClass sc = sm.getDeclaringClass();
							kid = new PDGRegion(body.hashCode(), sm,sc, new EnhancedUnitGraph(bb), body);
							kid.setParent(region);
							region.addChildRegion(kid);
				...
				else if(curNode instanceof ConditionalPDGNode)
				{
					...
					while(condItr.hasNext())
					{
						...
						if(!visited.contains(child))
						{
							visited.add(child);
							PDGRegion kid = null;
							if(child.getType().equals(PDGNode.Type.REGION))
							{
								kid = pdgpostorder(child, list);
								kid.setParent(region);
								region.addChildRegion(kid);
								//This sets the node from the old Region into a PDGRegion
							...
							else if(child.getType().equals(PDGNode.Type.CFGNODE)){

								Body bb = ((Block)child.getNode()).getBody();
								SootMethod sm = bb.getMethod();
								SootClass sc = sm.getDeclaringClass();
								kid = new PDGRegion(child.hashCode(), sm,sc, new EnhancedUnitGraph(bb), child);
								kid.setParent(region);
								region.addChildRegion(kid);
								//This sets the node from the old Region into a PDGRegion
	...
	{
		//return this.getSuccsOf(node);// wrong!
		List<PDGNode> toret = new ArrayList<PDGNode>();
