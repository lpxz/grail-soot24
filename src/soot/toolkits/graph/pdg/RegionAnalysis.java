...
public class RegionAnalysis{
	...
	private void findWeakRegions()
	{
		...
		DominatorTreeAdapter dta = new  DominatorTreeAdapter(this.m_dom);
	//	Utils.drawDirectedGraph(dta,  "random.dominator");
		...
		try{
			...
			if(this.m_blockCFG.getHeads().size() == 1)
			{
				...
				if (this.m_method.getName().equalsIgnoreCase("main")) {
					System.out.println("begin from now.......................");
				}
	...
	private void weakRegionDFS2(Block v, int r)
	{
		...
		//below.
		//System.out.println("visiting: block" + v.getIndexInMethod());
		//System.out.println("putting block"+ v.getIndexInMethod() + "into region:" + r);
		...
		if(u2!=null)
		{
			//System.out.println("its postdom parent: block" + u2.getIndexInMethod());
		}
		...
		for(int i = 0; i < children.size(); i++)
		{
			...
			if(u2 != null && u1.equals(u2))
			{
				//System.out.println("its child: block"+ u1.getIndexInMethod() +"==" + "postdom parent: "+ u2.getIndexInMethod() +" just dfs child" + u1.getIndexInMethod() );
			...
			{
				this.m_regCount++;
				Region newRegion= this.createRegion(this.m_regCount);
				this.m_regions.put(new Integer(this.m_regCount), newRegion);
				//System.out.println("its child: block"+ u1.getIndexInMethod() +"!=" + "postdom parent: "+ u2.getIndexInMethod() +" dfs child, and put it into new region!"  );
