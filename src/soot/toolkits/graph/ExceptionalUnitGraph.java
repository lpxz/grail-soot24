...
import pldi.locking.jin.GraphReachable;
...
public class ExceptionalUnitGraph extends UnitGraph implements ExceptionalGraph<Unit>
{
	...
	for (Iterator<Trap> trapIt = body.getTraps().iterator(); trapIt.hasNext(); ) {
	    ...
	    Unit end = units.getPredOf(trap.getEndUnit());
	  //   Iterator<Unit> unitIt = units.iterator(begin, end); // this statement is problematic
	    // cyclic control flow may map to multiple nodes, but the iterator returns null and reports exception
	    DirectedGraph dg = new BriefUnitGraph(body); 	    
	    Iterator unitIt= protectedSet(dg, begin, end).iterator();// end is inclusive, yes, correct
