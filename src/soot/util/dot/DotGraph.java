...
public class DotGraph implements Renderable{
  ...
  private HashMap<String, DotGraphNode> name2node;
  ...
  public DotGraph(String graphname) {
    ...
    this.name2node      = new HashMap<String, DotGraphNode>(100);
  ...
  public DotGraphEdge drawEdge4XRegionSucc(String from, String to) {

	    DotGraphNode src = drawNode(from);
	    
	    DotGraphNode dst = drawNode(to);
	    dst.setShape(DotGraphConstants.NODE_SHAPE_DIAMOND);
	    DotGraphEdge edge = new DotGraphEdge(src, dst);
	    edge.setLabel("->X");
  ...
  public DotGraphNode getNode(String name){
      DotGraphNode node = name2node.get(name);
      if (node == null) {
