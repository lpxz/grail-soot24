...
public class CriticalSectionAwareSideEffectAnalysis {
	static boolean opennesting= false;
	PointsToAnalysis pa;
	CallGraph cg;
	...
	public void findNTRWSets( SootMethod method ) {
		...
		for( Iterator sIt = method.retrieveActiveBody().getUnits().iterator(); sIt.hasNext(); ) {
			...
			if(!opennesting)// block it form the internal, if the opennesting is not enabled
			{
	...
	public RWSet nonTransitiveReadSet( SootMethod method ) {
		findNTRWSets( method );// foreach s in method, findNTR(method, s)
	...
	public CriticalSectionAwareSideEffectAnalysis( PointsToAnalysis pa, CallGraph cg, Collection<CriticalSection> criticalSections, ThreadLocalObjectsAnalysis tlo 
			,boolean opennestingPara) {
		opennesting=opennestingPara;
		this.pa = pa;
		this.cg = cg;
		
		this.tt = new TransitiveTargets( cg, null);
	...
	private RWSet ntReadSet( SootMethod method, Stmt stmt )
	{
		if( stmt instanceof AssignStmt ) {
			...
			if(r instanceof NewExpr) // IGNORE NEW STATEMENTS
				return null;
			return addValue( r, method, stmt );// same as my addValueToPointTo
		}
		return null;
	}
...
//				}
	...
	{
		// caller:tasea.readSet(tn.method, stmt, tn, uses);
		//===========uses is created at the entry, but no merge or return at all.
		// first get the tt's nt read, then itself's nt read set
		boolean ignore = false;
		if(stmt.containsInvokeExpr())
		{// directly skip!
		...
		Iterator<MethodOrMethodContext> targets = tt.iterator( stmt );//tt
		while( !ignore && targets.hasNext() )
		{
			SootMethod target = (SootMethod) targets.next();//foreach target
//			if( target.isNative() ) {
...
*/				}
				...
				{// note that all library functions have already been filtered out (by name) via the filter
				 // passed to the TransitiveTargets constructor.
					RWSet ntr = nonTransitiveReadSet(target); //for each s in target, NTRread(target,s)
					if( ntr != null ) {
//						uses.clear();
...
//=========================above is for each target, the below is for itself!
		RWSet ntr = ntReadSet( method, stmt );//critical
...
//=========================================stmt itself
		...
		ret.union( ntw );
//========================patch: because previously we did not handle the targets of this type!		
		if( stmt.containsInvokeExpr())
		{
			InvokeExpr ie = stmt.getInvokeExpr();
			SootMethod calledMethod = ie.getMethod();
	...
	//				else
						w = approximatedWriteSet(method, stmt, base, true);// over-approximated! all instance fields are added as "written"
					if( w != null )
						ret.union( w );
