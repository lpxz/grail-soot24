...
public class BodyCloneTestDriver {
	...
	public static void main(String[] args) throws IOException { // wjtp.tn
		...
		String argsOfToyW = "-f J -pp -cp .:/home/lpxz/eclipse/workspace/openjms/tmp -process-dir /home/lpxz/eclipse/workspace/openjms/lib/commons-logging-1.0.4.jar"; // java.lang.Math
			...
			protected void internalTransform(String phaseName, Map options) {
			  ...
			  while (scIt.hasNext()) {
				SootClass sootClass = (SootClass) scIt.next();
				 Iterator<SootField> fields =sootClass.getFields().iterator();
				   while (fields.hasNext()) {
					SootField sootField = (SootField) fields.next();
					System.err.println(sootClass.getName() + " field: " + sootField.getName());				
				}			
			}
               ...
               Iterator<Local> lcloneIT =bclone.getLocals().iterator();
               while (lIT.hasNext()) {
				Local local = (Local) lIT.next();
				Local lclone = (Local) lcloneIT.next();
				System.out.println(local.hashCode() + ":" + lclone.hashCode());
				if(local !=lclone)
				{
					
					System.out.println("not equal");
					Local l =Body.cloned2orig.get(lclone);
					if(local==l)
					{
						System.out.println("but after my processing, they are the smae");
						System.out.println(local.hashCode() + ":" + l.hashCode());
					}
				}
