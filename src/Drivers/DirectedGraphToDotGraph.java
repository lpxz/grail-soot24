 ...
 */
public class DirectedGraphToDotGraph {
	public NodeComparator comparator= null;
  public  HashMap name2node = null;//new HashMap();
   ...
   */
  public DirectedGraphToDotGraph() {
    setOnePage(true);
  ...
  private static class NodeRGComparator implements java.util.Comparator {
	    private DotRGNamer namer;

	    NodeRGComparator(DotRGNamer namer) {
	      this.namer = namer;
	    }
   ...
   */
  public DotGraph drawDG(DirectedGraph graph, String filename) {
     name2node = new HashMap();
	  DotGraph canvas = initDotGraph(filename);
    ...
    for (Iterator nodesIt = graph.iterator(); nodesIt.hasNext(); ) {
      ...
      String name = namer.getName(node);
...
//	if(node.toString().contains("if $z9 != 0 goto $i0 = i"))
//	{
//		System.out.println();
//	}
  ...
  public DotGraph drawDG4HRG(HRG graph, String filename) {
	     name2node = new HashMap();
		  DotGraph canvas = initDotGraph(filename);
	    DotNamer namer = new DotNamer((int)(graph.size()/0.7f), 0.7f);
	    NodeComparator comparator = new NodeComparator(namer);
	    ...
	    for (Iterator nodesIt = graph.iterator(); nodesIt.hasNext(); ) {
	      ...
	      for (Iterator succsIt = sortedIterator(graph.getSuccsOf(node), comparator);
		   succsIt.hasNext(); ) {
		Object succ = succsIt.next();
	    HRGnode succHRG = (HRGnode) succ;
	    if(succHRG.getHrgnType() == HRGnType.XREGION)
	    {
	    	canvas.drawEdge4XRegionSucc(namer.getName(node), namer.getName(succ));
	    }
	    else {
	    	canvas.drawEdge(namer.getName(node), namer.getName(succ));
		}
   ...
   */
  private DotGraph initDotGraph(String filename) {
    String graphname = filename;//;"cfg";
  ...
  public static class DotNamer extends HashMap {
    ...
    String getName(Object node) {
    	...
    	if(node instanceof EnterMonitorStmt)
    		prefix = "enter";
    	else if (node instanceof ExitMonitorStmt)
    		prefix = "exit";
  ...
  private static class DotRGNamer extends HashMap {
	    private int nodecount = 0;

	    DotRGNamer(int initialCapacity, float loadFactor) {
	      super(initialCapacity, loadFactor);
	    }
	    ...
	    String getName(Object node) {
	    	...
	    	{
	    		PDGNode n = (PDGNode) node;
	    		if(n.m_type == PDGNode.Type.REGION)
	    			return "Region"+  ((IRegion)n.m_node).getID();
	    		else
	    			return "Block"+  ((Block)n.m_node).getIndexInMethod();
...
//	      return index.toString();
  ...
  public  void generateName2Node( String filename) {
		// TODO Au
		...
		try {
			...
			Iterator keyIt= sortedIterator(name2node.keySet(), comparator ); 
			while (keyIt.hasNext()) {
				Object name = (Object) keyIt.next();
				Object node = name2node.get(name);
				out.write("name: "+ name + "\n");
...
public  DotGraph drawDepGraph(HashMutablePDG graph, String filename) {
    name2node = new HashMap();
    DotGraph canvas = initDotGraph(filename);
    DotRGNamer namer = new DotRGNamer((int)(graph.size()/0.7f), 0.7f);
    NodeRGComparator comparator = new NodeRGComparator(namer);
    ...
    for (Iterator nodesIt = graph.iterator(); nodesIt.hasNext(); ) {
      ...
      for (Iterator succsIt = sortedIterator(graph.getSuccsOf(node), comparator);
	   succsIt.hasNext(); ) {
	Object succ = succsIt.next();
	String startname =namer.getName(node);
	String childname = namer.getName(succ);
