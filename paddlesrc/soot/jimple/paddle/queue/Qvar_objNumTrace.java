...
public final class Qvar_objNumTrace extends Qvar_obj {
    ...
    public void add(VarNode _var, AllocNode _obj) {
        ...
        for (Iterator it = readers.iterator(); it.hasNext(); ) { Rvar_objNumTrace reader = (Rvar_objNumTrace) it.next();
                                                                 reader.add(in); } }
    
    public void add(final jedd.internal.RelationContainer in) { throw new RuntimeException(); }
    
    public Rvar_obj reader(String rname) { Rvar_obj ret = new Rvar_objNumTrace(name + ":" + rname, this);
                                           readers.add(ret);
                                           return ret; }
}
