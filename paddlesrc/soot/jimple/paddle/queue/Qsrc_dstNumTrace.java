...
public final class Qsrc_dstNumTrace extends Qsrc_dst {
    ...
    public void add(VarNode _src, VarNode _dst) {
        ...
        for (Iterator it = readers.iterator(); it.hasNext(); ) { Rsrc_dstNumTrace reader = (Rsrc_dstNumTrace) it.next();
                                                                 reader.add(in); } }
    
    public void add(final jedd.internal.RelationContainer in) { throw new RuntimeException(); }
    
    public Rsrc_dst reader(String rname) { Rsrc_dst ret = new Rsrc_dstNumTrace(name + ":" + rname, this);
                                           readers.add(ret);
                                           return ret; }
}
