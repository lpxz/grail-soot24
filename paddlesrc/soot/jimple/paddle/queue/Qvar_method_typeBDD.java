...
public final class Qvar_method_typeBDD extends Qvar_method_type {
    public Qvar_method_typeBDD(String name) {
        super(name);
    }
    ...
    public void add(VarNode _var,
                    SootMethod _method,
                    Type _type) {
        add(new jedd.internal.RelationContainer(new Attribute[] { A_var.v(), A_method.v(), A_type.v() },
    ...
    public void add(final jedd.internal.RelationContainer in) {
        ...
        for (Iterator it =
            ...
            Rvar_method_typeBDD reader =
              (Rvar_method_typeBDD)
                it.next();
            reader.add(new jedd.internal.RelationContainer(new Attribute[] { A_type.v(), A_var.v(), A_method.v() },
                                                           new PhysicalDomain[] { T1.v(), V1.v(), MS.v() },
                                                           "reader.add(in) at Qvar_method_typeBDD.jedd:40,12-18",
                                                           in));
        }
    }
    
    public Rvar_method_type reader(String rname) {
        Rvar_method_type ret =
          new Rvar_method_typeBDD(name +
                                  ":" +
                                  rname,
                                  this);
        readers.add(ret);
