...
public class Qsrc_dst_fldDebug extends Qsrc_dst_fld {
    public Qsrc_dst_fldDebug(String name) {
        super(name);
    }
    
    private Qsrc_dst_fldBDD bdd =
      new Qsrc_dst_fldBDD(name +
                          "bdd");
    
    private Qsrc_dst_fldSet trad =
      new Qsrc_dst_fldSet(name +
                          "set");
    ...
    public void add(final jedd.internal.RelationContainer in) {
        Iterator it =
          new jedd.internal.RelationContainer(new Attribute[] { A_fld.v(), A_src.v(), A_dst.v() },
                                              ...
                                              ("in.iterator(new jedd.Attribute[...]) at Qsrc_dst_fldDebug.je" +
                                               "dd:40,22-24"),
    ...
    public Rsrc_dst_fld reader(String rname) {
        return new Rsrc_dst_fldDebug((Rsrc_dst_fldBDD)
                                       bdd.reader(rname),
                                     (Rsrc_dst_fldSet)
                                       trad.reader(rname),
                                     name +
